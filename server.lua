...
AddEventHandler('neey_keys:removeKey', function(plate)
    ...
    if found then
        exports.ox_inventory:RemoveItem(id, 'carkey', 1, metadata)
    end
    vehicleKeys[plate] = false
end)

if ( GetCurrentResourceName() ~= "neey_keys" ) then
    print( "^B£¥D: Nazwij skrypt neey_keys, w przeciwnym razie niektóre skrypty b¹dŸ funkcjê mog¹ dzia³aæ nieprawid³owo^0\n\n" )
  end